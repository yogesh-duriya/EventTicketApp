...
public class PrevFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (!fragmentResume && fragmentVisible && eventArr != null && eventArr.size() == 0) {   //only when first time fragment is created
            organizerData(OrganizeProfileActivity.organizer_id, "3");
        }
        return view;
    }
    ...
    private void setAdapter(ArrayList<SomethingInterestingDataPojo> arr) {
        ctx.adapterPrevious = new EventsListAdapter(ctx, arr, "OrganizerProfile");
        if (arr != null) {
            rv_package_list.setAdapter(ctx.adapterPrevious);
        }
    }
}
